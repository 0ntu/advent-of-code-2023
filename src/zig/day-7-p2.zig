...
const Hand = struct {
    ...
    pub fn getHandType(self: *const Self) !HandType {
        ...
        std.debug.panic("card len: {d}\n\ncard: {any}\n", .{num_per_unique.len, num_per_unique});
    }
    ...
    fn uniqueFromHand(self: *const Self, allocator: std.mem.Allocator) ![]UniqueCard {
        ...
        over_held_cards: for (self.cards) |card| {
            ...
            //find if card is already in unique_cards
        ...
        //find joker cards, and remove jokers cards
        var joker_cards: ?UniqueCard = null;
        for (unique_cards.items, 0..unique_cards.items.len) |unique_card, i| {
            if (unique_card.value == 'J') {
                joker_cards = unique_cards.swapRemove(i);
            }
        }

        //add joker_cards num_in_hand to the highest unique_Card
        if (joker_cards) |jokers| {
            //special case, if hand is full of jokers
            if (jokers.num_in_hand == 5) {
                try unique_cards.append(jokers);
                return try unique_cards.toOwnedSlice();
            }


            var highest_num_in_hand = unique_cards.items[0].num_in_hand;
            var highest_num_in_hand_index: usize = 0;

            for (0..unique_cards.items.len) |i| {
                if (unique_cards.items[i].num_in_hand > highest_num_in_hand) {
                    highest_num_in_hand_index = i;
                    highest_num_in_hand = unique_cards.items[i].num_in_hand;
                }
            }

            unique_cards.items[highest_num_in_hand_index].num_in_hand += jokers.num_in_hand;
        }
...
pub fn main() !void {
    ...
    for (hands.items, 0..hands.items.len) |hand, i| {
        //hands are sorted worst to best
...
test "hand types" {
    ...
    try std.testing.expect(total_winnings == 5905);
}
